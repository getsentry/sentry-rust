...
pub fn create_testable_hub(options: ClientOptions) -> Hub {
    Hub::new(Some(Arc::new(Client::testable(TEST_DSN.clone(), options))), Arc::new(Default::default()))
}

/// Fetches all events from a testable hub.
pub fn fetch_events(hub: &Hub) -> Vec<Event<'static>> {
    hub.client().expect("need a hub with client").transport().fetch_and_clear_events()
}
...
/// Runs some code in the context of the default test hub and returns the
/// captured events.
pub fn with_captured_events_options<F: FnOnce()>(f: F, options: ClientOptions) -> Vec<Event<'static>> {
