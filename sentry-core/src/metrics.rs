...
macro_rules! metric_emit {
    ...
    ($type:expr, $name:expr, $value:expr, $($rest:tt)+) => {{
        ...
        let mut unit: Option<::std::borrow::Cow<'static, str>> = None;
...
macro_rules! metric_count {
    ($name:expr, $value:expr $(, $($rest:tt)+)?) => {
        $crate::metric_emit!($crate::protocol::MetricType::Counter, $name, $value $(, $($rest)+)?)
    };
}
...
macro_rules! metric_gauge {
    ($name:expr, $value:expr $(, $($rest:tt)+)?) => {
        $crate::metric_emit!($crate::protocol::MetricType::Gauge, $name, $value $(, $($rest)+)?)
    };
}
...
macro_rules! metric_distribution {
    ($name:expr, $value:expr $(, $($rest:tt)+)?) => {
        $crate::metric_emit!($crate::protocol::MetricType::Distribution, $name, $value $(, $($rest)+)?)
    };
}
